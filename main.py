...
def init_subdirs(MSI_dir, MSS_dir, cohort_name = '', sample_size=5000, n_jobs=1, validation=True, subsets=True):
    # creates the save path for the results of the texture analysis - found in same directory as code
    if os.path.exists(os.path.join(os.getcwd(),(cohort_name + '_texture_analysis_results'))) == False:
        os.mkdir(cohort_name + '_texture_analysis_results')
    save_path = os.path.join(os.getcwd(),(cohort_name + '_texture_analysis_results'))
...
def main(MSI_dir, MSS_dir, sample_size=5000, n_jobs=1, cohort_name='', validation=False, subsets=True):
    ...
    if validation == True:
        MSI_subsets_dir, MSS_subsets_dir, MSI_validation_dir, MSS_validation_dir, save_path = init_subdirs(MSI_dir, MSS_dir, cohort_name=cohort_name, sample_size=sample_size, n_jobs=n_jobs, validation=validation, subsets=subsets)
    else:
        MSI_subsets_dir, MSS_subsets_dir, save_path = init_subdirs(MSI_dir, MSS_dir, cohort_name=cohort_name, sample_size=sample_size, n_jobs=n_jobs, validation=validation, subsets=subsets)
    ...
    if validation == True:
        y_pred = plot_predict(X_train,X_test, y_train, y_test, data, os.path.join(save_path, 'MSI_validation_subsets'), os.path.join(save_path, 'MSS_validation_subsets'), title=('Prediction of MSI in ' + cohort_name), validation=validation)
    else:
        y_pred = plot_predict(X_train,X_test, y_train, y_test, data, title=('Prediction of MSI in ' + cohort_name), validation=validation)
    per_patient(X_test, y_pred, y_test, test_ind, title=('Per Patient Prediction of MSI in ' + cohort_name), validation=validation)
